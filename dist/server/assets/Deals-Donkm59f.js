...
import { u as useAuth, I as Icons, D as DealsSkeleton } from "../entry-server.js";
...
const Deals = ({ isInDashboard = false }) => {
  ...
  const categoryStatsQuery = useCategoryStatsQuery(Array.isArray(deals) ? deals : []);
  const categoryStats = categoryStatsQuery.data || /* @__PURE__ */ new Map();
  const categoryInfoQuery = useCategoryInfoQuery(Array.isArray(companies) ? companies : []);
  ...
  const mainContent = /* @__PURE__ */ jsxs(PageWrapper, { isDashboard: isDashboardContext, children: [
    /* @__PURE__ */ jsx(
      HeaderSection,
      {
        companiesCount: companies.length,
  ...
  return isDashboardContext ? mainContent : mainContent;
};
const PageWrapper = ({
  isDashboard,
  children
...
const HeaderSection = ({ companiesCount, categoriesCount, isDashboard, isLoading = false }) => /* @__PURE__ */ jsx("div", { className: "text-center mb-6xl animate-fade-in-up", children: /* @__PURE__ */ jsxs("div", { className: "glass-enhanced rounded-3xl container-p-2xl border border-divider/30 shadow-enhanced-lg", children: [
  ...
  /* @__PURE__ */ jsx("p", { className: `text-foreground/70 mx-auto ${isDashboard ? "text-sm mb-md" : "text-xl mb-xl"}`, children: isLoading ? "Loading exclusive offers with real-time ratings..." : `Curated offers from top platforms across ${categoriesCount} categories` }),
