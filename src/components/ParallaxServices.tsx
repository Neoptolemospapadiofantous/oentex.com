  ...
  return (
    <div ref={containerRef} className="my-3xl relative">
      ...
      <div 
        ...
        style={{
          backgroundImage: `
            radial-gradient(circle at 30% 40%, rgba(var(--heroui-primary), 0.1) 0%, transparent 50%),
            radial-gradient(circle at 70% 60%, rgba(var(--heroui-secondary), 0.1) 0%, transparent 50%)
          `
        }}
      />
      ...
      <div className="grid lg:grid-cols-2 gap-xl">
        {services.map((service, index) => (
          <div 
            ...
            className={`card-feature hover:bg-surface/70 transition-all duration-500 ${
              isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'
            }`}
            style={{ transitionDelay: `${0.3 + index * 0.1}s` }}
          >
            <div className="flex items-start space-x-4 my-lg">
              ...
              <div>
                <h3 className="text-xl font-semibold text-text my-sm">{service.title}</h3>
            ...
            <div className="bg-gradient-to-r from-warning/5 to-accent/5 border border-warning/20 rounded-lg p-sm flex items-start space-x-2">
              <Icons.warning className="w-4 h-4 text-warning flex-shrink-0 mt-0.5" />
