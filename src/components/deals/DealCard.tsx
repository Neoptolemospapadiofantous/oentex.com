// src/components/deals/DealCard.tsx - Enhanced with In-Card Terms Display
...
export const DealCard: React.FC<DealCardProps> = ({ 
  ...
  // Handle mouse leave from card to hide terms
  useEffect(() => {
  ...
  const renderStars = (rating: number, size: string = 'w-4 h-4') => {
   ...
   const renderTermsContent = (term: ParsedTerm, index: number) => {
     switch (term.type) {
       case 'header':
         return (
           <div key={index} className="text-center py-md animate-in fade-in slide-in-from-top-2 duration-500" style={{ animationDelay: `${index * 100}ms` }}>
       ...
       case 'section':
         ...
         return (
           <div key={index} className={`${marginTop} mb-sm animate-in fade-in slide-in-from-left-2 duration-400`} style={{ animationDelay: `${index * 100}ms` }}>
             ...
             <div className="w-full h-px bg-gradient-to-r from-border/30 to-transparent mt-sm group-hover:from-primary/50 transition-colors duration-300" />
           </div>
         )

       case 'bullet':
         return (
           <div key={index} className="flex items-start gap-sm ml-md py-xs hover:bg-content2/30 rounded-md container-px-sm transition-all duration-300 group animate-in fade-in slide-in-from-left-1 duration-300" style={{ animationDelay: `${index * 100}ms` }}>
       ...
       case 'paragraph':
         return (
           <p key={index} className="text-foreground/80 leading-relaxed text-xs hover:bg-content2/20 rounded-md container-p-sm transition-all duration-300 animate-in fade-in slide-in-from-bottom-1 duration-300 hover:scale-105" style={{ animationDelay: `${index * 100}ms` }}>
             {term.content}
           </p>
         )
  ...
  return (
    <div 
      ref={cardRef}
      className="group bg-surface rounded-2xl border border-border hover:border-primary/30 transition-all duration-300 hover:shadow-xl hover:-translate-y-1 overflow-hidden flex flex-col h-full relative"
      onMouseEnter={() => setIsCardHovered(true)}
      onMouseLeave={() => setIsCardHovered(false)}
    >
       {/* Terms Overlay - Shows when showTerms is true */}
       {showTerms && (
         <div 
           ref={termsRef}
           className="absolute inset-0 bg-content1/95 backdrop-blur-sm z-20 overflow-hidden animate-in fade-in zoom-in-95 slide-in-from-top-2 duration-500 flex flex-col"
         >
          {/* Terms Header */}
          <div className="bg-gradient-to-r from-primary/10 via-secondary/5 to-primary/10 p-lg border-b border-border/30 flex-shrink-0">
          ...
          {/* Terms Content - Middle section that grows and scrolls */}
          <div className="flex-1 overflow-y-auto p-lg scrollbar-thin scrollbar-thumb-primary/30 scrollbar-track-transparent">
            {parsedTerms.length > 0 ? (
              <div className="space-y-sm">
                {parsedTerms.map(renderTermsContent)}
              </div>
            ) : (
              <div className="text-center py-xl animate-in fade-in zoom-in-95 duration-500">
          ...
          {/* Terms Footer with Action Buttons */}
          <div className="bg-content2/95 backdrop-blur-sm border-t border-border/30 p-lg flex-shrink-0">
            <div className="flex gap-sm">
              <button
                onClick={() => setShowTerms(false)}
                className="flex-1 container-px-md container-py-sm text-foreground/70 border border-border rounded-lg hover:bg-content2 hover:text-foreground transition-all duration-300 font-medium text-sm hover:scale-105 active:scale-95"
              >
                Close
              </button>
              <button
                ...
                className="flex-1 container-px-md container-py-sm bg-gradient-to-r from-primary to-secondary text-white rounded-lg hover:shadow-lg hover:scale-105 active:scale-95 transition-all duration-300 font-medium text-sm flex items-center justify-center gap-xs"
              >
      ...
      {/* Original Card Content - Visible when terms is closed */}
      ...
      {/* Action Buttons - Always at bottom, hidden when terms is open */}
       <div className={`p-lg pt-0 mt-auto transition-opacity duration-300 ${showTerms ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
         {/* Deal metadata */}
         ...
         {/* Terms & Conditions Link - In-card trigger */}
         <div className="mt-md flex justify-center">
          <button
            onClick={handleTermsClick}
            className="text-xs text-textSecondary hover:text-primary transition-all duration-300 underline hover:no-underline container-px-md container-py-sm rounded-lg hover:bg-background/50 focus:outline-none focus:ring-2 focus:ring-primary/50 hover:scale-105 active:scale-95"
          >
            Terms & Conditions
          </button>
        </div>
      ...
      {/* Hover effect overlay - hidden when terms is open */}
